...
func main() {
	var (
		...
		h  = flag.Bool("help", false, "Show usage information")
	)

	program := "go run scripts/release.go"

	flag.Usage = func() {
		fmt.Printf("Usage: %s -type=<bump_type> [options]\n\n", program)
		fmt.Printf("Release tool for Go libraries with semantic versioning.\n\n")
		fmt.Printf("Options:\n")
		flag.PrintDefaults()
		fmt.Printf("\nExamples:\n")
